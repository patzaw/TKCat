...
add_feature_def <- function(
  ...
  stopifnot(
    ...
    all(unlist(lapply(names(properties), function(pn) {
      ...
      if (!res) {
        message(sprintf("%s: Wrong property info", pn))
      }
      res <- res &&
        is.character(p$type) &&
        length(p$type) == 1 &&
      ...
      if (!res) {
        message(sprintf("%s: Wrong type", pn))
      }
      res <- res &&
        length(p$mandatory) == 1 &&
        is.logical(p$mandatory) &&
        !is.na(p$mandatory)
      if (!res) {
        message(sprintf("%s: Wrong mandatory value", pn))
      }
      res <- res &&
      ...
      if (!res) {
        message(sprintf("%s: Wrong measurement value", pn))
      }
      res <- res &&
      ...
      if (!res) {
        message(sprintf("%s: Wrong description value", pn))
      }
      return(res)
    })))
  )
...
add_table_def <- function(
  ...
  stopifnot(
    ...
    length(collection) == 1,
    is.character(mandatory_features),
    length(mandatory_features) > 0,
...
list_table_features <- function(kmr, tables = NULL) {
  ...
  if (length(tables) == 0) {
    ...
    toRet <- join_mdb_tables(
      fkmr,
      setdiff(names(fkmr), "Feature_properties")
    )
    toRet <- toRet %>%
