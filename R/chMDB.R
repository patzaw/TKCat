...
as_chMDB <- function(
  ...
  if (!is.null(collectionMembers) && nrow(collectionMembers) > 0) {
    ...
    toAdd <- setdiff(
      toAdd,
      dplyr::pull(list_chTKCat_collections(tkcon), "title")
    )
    for (col in toAdd) {
...
#' @export
#'
filter_with_tables.chMDB <- function(
  x,
  tables,
  checkTables = TRUE,
  by = 10^5,
...
filter_mdb_matrix.chMDB <- function(x, tableName, ...) {
  ...
  if (.is_ch_large_matrix(qr)) {
    ## Large matrix ----

    rt <- qr$table[which(qr$info == "rows")]
    ct <- qr$table[which(qr$info == "columns")]
    vt <- qr$table[which(qr$info == "all_values")]
    ...
    toRet <- matrix(
      ...
      ncol = nrow(fcj),
      byrow = FALSE,
      dimnames = list(fri$name, fcj$name)
    )
  ...
  dimcol <- intersect(
    dimcol$name,
    c("___COLNAMES___", "___ROWNAMES___")
  )
  ...
  # query <- tquery
  queries <- tquery
  if (length(sel) > 0 && length(mtables) > 1) {
    for (i in 2:length(mtables)) {
      if (is.na(sel[1])) {
        tquery <- paste(
          sprintf(
            queryTemplate,
...
###############################################################################@
## Helpers ----
.write_chTables.chMDB <- function(x, con, dbName, by = 10^7, ...) {
  dm <- data_model(x)
  for (tn in names(x)) {
    message(tn)
    tm <- dm[[tn]]
    if (ReDaMoR::is.MatrixModel(tm)) {
      ...
      if (.is_ch_large_matrix(stl)) {
        ## Sparse matrix or large matrix ----

        ## Columns and rows
        rquery <- sprintf(
          "SELECT i, name FROM %s", # ORDER BY i",
          sprintf('`%s`.`%s`', tndb, stl$table[which(stl$info == "rows")])
        )
        rowNames <- get_query(
          x,
          rquery,
        ...
        cquery <- sprintf(
          "SELECT j, name FROM %s", # ORDER BY j",
          sprintf('`%s`.`%s`', tndb, stl$table[which(stl$info == "columns")])
        )
        colNames <- get_query(
          x,
          cquery,
        ...
        write_MergeTree(
          con = con,
          dbName = dbName,
          tableName = stl$table[which(stl$info == "rows")],
          value = rowNames,
        ...
        write_MergeTree(
          con = con,
          dbName = dbName,
          tableName = stl$table[which(stl$info == "columns")],
          value = colNames,
        ...
        valTable <- stl$table[which(stl$info %in% c("values", "all_values"))]
        write_MergeTree(
          con = con,
          dbName = dbName,
          tableName = valTable,
          ...
          rtypes = c("i" = "integer", "j" = "integer", "x" = vtype),
          nullable = nullable,
          sortKey = c("i", "j")
        )
        chTables <- list_tables(
    ...
    } else {
      {
        ...
        while (nrow(toWrite) > 0) {
          ...
          toWrite <- data_tables(x, dplyr::all_of(tn), skip = r, n_max = by)[[
            1
          ]]
          r <- r + nrow(toWrite)
        }
...
.is_ch_large_matrix <- function(d) {
  identical(sort(colnames(d)), sort(c("info", "table"))) &&
    identical(sort(d$info), sort(c("columns", "rows", "all_values")))
}
...
.get_ch_mtable <- function(
  ...
  if (ReDaMoR::is.MatrixModel(tableModel)) {
    ...
    if (.is_chMM(qr)) {
      ...
      toRet <- Matrix::sparseMatrix(
        i = as.integer(values$i),
        j = as.integer(values$j),
        x = values$x,
    ...
    if (.is_ch_large_matrix(qr)) {
      ...
      if (getsubtab) {
        warning(
          "Be careful when using skip and n_max parameters: ",
          "Subsetting a large matrix from clickhouse may return ",
          "inconsistent NA values"
        )
      }

      vquery <- sprintf(
        "SELECT i, j, x FROM %s", # ORDER BY j, i",
        sprintf('`%s`.`%s`', tdb, qr$table[which(qr$info == "all_values")])
      )
      vquery <- paste(
        vquery,
        sprintf("LIMIT %s, %s", skip, n_max)
      ...
      toRet <- matrix(
        c(
          ...
          rep(
            NA,
            length(unique(values$i)) * length(unique(values$j)) - nrow(values)
          )
        ),
        nrow = length(unique(values$i)),
        ncol = length(unique(values$j)),
        byrow = FALSE,
        dimnames = list(rowNames$name, colNames$name)
      )
      return(toRet)
...
.ch_filtByConta <- function(d, fdb, fk, by = 10^5) {
  ...
  .contaminate <- function(tn) {
    ...
    nfk <<- nfk %>%
      dplyr::anti_join(
        dplyr::select(fkf, "from", "to"),
        by = c("from", "to")
      ) %>%
      dplyr::anti_join(
        dplyr::select(fkt, "from", "to"),
        by = c("from", "to")
      )
    fkl <- dplyr::bind_rows(
      fkf,
      fkt %>%
        dplyr::rename("from" = "to", "ff" = "tf", "to" = "from", "tf" = "ff")
    ) %>%
      dplyr::distinct()
    if (nrow(fkl) > 0) {
      for (i in 1:nrow(fkl)) {
          ...
          if (!is.null(qr) && .is_chMM(qr)) {
            fv <- lapply(1:length(fkl$tf[[i]]), function(j) {
              ...
              if (inherits(d[[tn]], c("matrix", "Matrix"))) {
                if (tm$fields$type[which(tm$fields$name == tf)] == "row") {
                  tv <- rownames(d[[tn]])
                } else {
                  if (
                    tm$fields$type[which(tm$fields$name == ntf)] == "column"
                  ) {
                    tv <- colnames(d[[tn]])
                  } else {
                    stop(sprintf(
          ...
          fv <- lapply(1:length(fkl$tf[[i]]), function(j) {
            ...
            if (inherits(d[[tn]], c("matrix", "Matrix"))) {
              if (tm$fields$type[which(tm$fields$name == tf)] == "row") {
                ...
                if (tm$fields$type[which(tm$fields$name == ntf)] == "column") {
                  tv <- colnames(d[[tn]])
                } else {
                  stop(sprintf(
                    paste(
            ...
            if (ntm$fields$type[which(ntm$fields$name == ntf)] == "row") {
