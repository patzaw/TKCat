...
.build_etkc_server_default <- function(
  ...
  function(input, output, session) {
    ...
    shiny::observe({
      shiny::withProgress(message = "Getting list of MDBs", expr = {
    ...
    shiny::observe({
      ...
      if (utils::object.size(tmp) > 2^19) {
        toShow <- toShow[
          1:max(c(
            1,
            ceiling(nrow(toShow) * (2^19 / utils::object.size(tmp)))
          )),
        ]
      }
      tabSubSet(toShow)
          ...
          future::future(
            {
              ...
              readr::write_tsv(
                data_tables(mdb, dplyr::all_of(sel))[[1]],
                tf
              )
              file.rename(tf, f)
              return(fname)
          ...
          shiny::tags$ul(
            ...
            if (length(k$ports) > 0) {
              shiny::tags$li(
                ...
                do.call(
                  shiny::tags$ul,
                  lapply(names(k$ports), function(n) {
                    shiny::tags$li(
                      shiny::tags$strong(n),
                      ":",
                      k$ports[n]
