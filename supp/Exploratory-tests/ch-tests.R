...
library(ReDaMoR)
TKCat:::writeMergeTree(
  con,
  "default",
  "d",
  d,
  rtypes = c("a" = "integer", "b" = "integer")
)


dbSendQuery(
  con,
  'create row policy dpgorr on d as restrictive for select to pgodard'
)
dbSendQuery(
  con,
  'create row policy dpgo on d as permissive using b>=18 to pgodard'
)
dbSendQuery(
  con,
  'create row policy dpgor on d as permissive for select using 1 to pgodard'
)
