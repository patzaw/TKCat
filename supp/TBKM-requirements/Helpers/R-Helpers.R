...
add_collibra_metadata <- function(
  ...
  dm$`___Collibra___`$fields$comment <- finfo$feature.description[match(
    paste(dm$`___Collibra___`$fields$name, "(Collibra)"),
    finfo$feature
  )]
  ...
  dm$`___Collibra_dds___`$fields$comment <- finfo$feature.description[match(
    paste(dm$`___Collibra_dds___`$fields$name, "(Collibra)"),
    finfo$feature
  )]
  ...
  toRet <- add_km_table(
    toRet,
    kmr,
    name = "___Collibra_dds___",
    type = "collibra drug development stage",
    features = lapply(
...
list_beid_scopes <- function(
  ...
  possible_types <- matrix(
    c(
      ...
      "beid ref",
      "biological entities",
      "Table of biological entity identifiers",
      NA
    ),
    ncol = 3,
    byrow = TRUE,
  ...
  if ("biological entities" %in% found$type) {
    ...
    table_fk <- unlist(lapply(
      dm[[table]]$foreignKeys,
      function(x) x$refTable
    ))
