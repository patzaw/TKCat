...
tbkm <- add_property_values(
  ...
  values = c(
    ...
    "UCB Clinical Samples",
    "Experiments",
    "Experiments/Multi omics",
    "Assays and reference data",
    "Molecules"
  )
)
...
tbkm <- add_property_values(
  ...
  values = sort(setdiff(
    BED::bedCall(
      neo2R::cypher,
      query = "MATCH (n:BEID) RETURN DISTINCT n.database AS db"
    )$db,
    c("BEDTech_gene", "BEDTech_transcript")
  ))
)
tbkm <- add_property_values(
  tbkm,
